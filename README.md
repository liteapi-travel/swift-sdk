...
To add the Swift SDK package, follow these steps:

1- Copy this Repository URL:

```sh
    https://github.com/liteapi-travel/swift-sdk
```

2- Open your project in Xcode. go to "File" in the menu bar and click "Add Packages"

3- In the "Add Packages" window, paste the copied repository URL into the search bar.

4- Select the package and click the "Add Package" button to start the integration process.
...
```swift
import LiteAPIClient

// Add this line within the appropriate context, such as the app's entry point or configuration setup.
LiteAPIClientAPI.customHeaders["X-API-Key"] = "YOUR_API_KEY"
```
...
The getCitiesByCountry function returns a list of city names from a specific country. The country codes must be in ISO-2 format. To get the country codes in ISO-2 for all countries please use the getCountries function. 

*  <h4 style="color:#9155fd; font-weight: 800;"> Example :</h4>
```swift
    let countryCode = "US"
    StaticDataAPI.getCitiesByCountry(countryCode: countryCode) {response, error in
...
*  <h4 style="color:#9155fd; font-weight: 800;"> Example :</h4>
```swift
    StaticDataAPI.getCountries() {response, error in
        if let response = response {dump(response)}
    }
```
*  <h4 style="color:#9155fd; font-weight: 800;"> Parameters :</h4>
...
*  <h4 style="color:#9155fd; font-weight: 800;"> Example :</h4>
```swift
    StaticDataAPI.getCurrencies() {response, error in
        if let response = response {dump(response)}
    }
```
*  <h4 style="color:#9155fd; font-weight: 800;"> Parameters :</h4>
...
```swift
    let countryCode = "IT";
    let cityName = "Rome";
    StaticDataAPI.getHotels(countryCode: countryCode, cityName: cityName) {response, error in
        if let response = response {dump(response)}
    }
```
...
```swift
    ...
    let offset = 1000;
    let limit = 1000;
    let longitude = "-115.16988";
    let latitude = "36.12510";
    let distance = 1000;

    StaticDataAPI.getHotels(countryCode: countryCode, cityName: cityName, offset: offset, limit: limit, longitude: longitude, latitude: latitude, distance: distance) {response, error in
        if let response = response {dump(response)}
    }
```
...
```swift
    let hotelId = "lp24373"
    StaticDataAPI.getHotelDetails(hotelId:hotelId) {response, error in
        if let response = response {dump(response)}
    }
```
*  <h4 style="color:#9155fd; font-weight: 800;"> Parameters :</h4>
...
*  <h4 style="color:#9155fd; font-weight: 800;"> Example :</h4>
```swift
    StaticDataAPI.getIataCodes() {response, error in
        if let response = response {dump(response)}
    }
```
*  <h4 style="color:#9155fd; font-weight: 800;"> Parameters :</h4>
...
*  <h4 style="color:#9155fd; font-weight: 800;"> Example :</h4>
```swift
    let hotelIds:[String] = ["lp3803c","lp1f982","lp19b70","lp19e75"];
    let checkin = "2023-11-15";
    let checkout = "2023-11-16";
    let currency = "USD";
    let guestNationality = "US";
    let adults = 1;
    SearchAPI.getMinimumRates(hotelIds: hotelIds, checkin: checkin, checkout: checkout, currency: currency, guestNationality: guestNationality, adults: adults) {response, error in
        if let response = response {dump(response)}
    }
```
...
```swift
    ...
    let adults = 1;
    //Optional values
    let children:[String] = ["12","9","4"];
    let guestId = "traveler1";
    SearchAPI.getMinimumRates(hotelIds: hotelIds, checkin: checkin, checkout: checkout, currency: currency, guestNationality: guestNationality, adults: adults,children:children,guestId:guestId) {response, error in
        if let response = response {dump(response)}
    }
```
...
```swift
    ...
    let guestNationality = "US";
    let adults = 1;
    SearchAPI.getFullRates(hotelIds: hotelIds, checkin: checkin, checkout: checkout, currency: currency, guestNationality: guestNationality, adults: adults) {response, error in
        if let response = response {dump(response)}
    }
```
...
```swift
    ...
    let children:[String] = ["12","9","4"];
    let guestId = "traveler1";
    SearchAPI.getFullRates(hotelIds: hotelIds, checkin: checkin, checkout: checkout, currency: currency, guestNationality: guestNationality, adults: adults,children:children,guestId:guestId) {response, error in
        if let response = response {dump(response)}
    }
```
*  <h4 style="color:#9155fd; font-weight: 800;"> Parameters :</h4>
...
*  <h4 style="color:#9155fd; font-weight: 800;"> Example :</h4>
```swift 
    let rateId = "";
    BookAPI.prebook(rateId:rateId) {response, error in
        if let response = response {dump(response)}
    }
```
*  <h4 style="color:#9155fd; font-weight: 800;"> Parameters :</h4>
...
```swift 
    let prebookId = "0f5NwOvxQ";
    let guestFirstName = "Kim";
    let guestLastName = "James";
    let guestEmail = "test@nlite.ml";

    let holderName = "Kim James";
    let paymentMethod = "CREDIT_CARD";
    let cardNumber  = "4242424242424242";
    let expireMonth = "11";
    let expireYear = "23";
    let cvc = "123";
    BookAPI.book(prebookId:prebookId, guestFirstName:guestFirstName, guestLastName:guestLastName, guestEmail:guestEmail, holderName:holderName, paymentMethod:paymentMethod, cardNumber:cardNumber, expireMonth:expireMonth, expireYear:expireYear, cvc:cvc) {response, error in
        if let response = response {dump(response)}
    }
```
...
```swift 
    ...
    let holderName = "Kim James";
    let paymentMethod = "STRIPE_TOKEN";
    let token = "NGVEVSHIVMVISKNIJNEYSSMKZEVKQSEI5CVURC"
    BookAPI.book(prebookId:prebookId, guestFirstName:guestFirstName, guestLastName:guestLastName, guestEmail:guestEmail, holderName:holderName, paymentMethod:paymentMethod, token:token) {response, error in
        if let response = response {dump(response)}
    }
```
*  <h4 style="color:#9155fd; font-weight: 800;"> Parameters :</h4>
...
```swift 
    ...
    BookingManagementAPI.getBookingListByGuestId(guestId: guestId) {response, error in
        if let response = response {dump(response)}
    }
```
...
```swift 
    let bookingId = "K8Hvb-85O"
    BookingManagementAPI.retrievedBooking(bookingId:bookingId) {response, error in
        if let response = response {dump(response)}
    }
```
*  <h4 style="color:#9155fd; font-weight: 800;"> Parameters :</h4>
...
```swift 
    let bookingId = "K8Hvb-85O"
    BookingManagementAPI.cancelBooking(bookingId: bookingId) {response, error in
        if let response = response {dump(response)}
    }
```
*  <h4 style="color:#9155fd; font-weight: 800;"> Parameters :</h4>
...
# Example Project
To see an example project demonstrating how to integrate the SDK, visit [liteAPI-nodejs-sdk-examples](https://github.com/liteapi-travel/swift-sdk-examples)